...
if len(dir) != 0:
    ...
    climate_res = plio_temp.x[1] - plio_temp.x[0]
    print('climate res ' + str(climate_res))
    if plotfile_res > climate_res:
        downsample_interval = int(plotfile_res / climate_res)
        temp_uncorrected = temp_uncorrected[:, ::downsample_interval, ::downsample_interval]
        precip = precip[:, ::downsample_interval, ::downsample_interval]
        plioGCM_elevation = plio_elev.orog[::downsample_interval, ::downsample_interval]
        plioGCM_elevation = np.nan_to_num(plioGCM_elevation, nan=np.nanmin(plioGCM_elevation))
        controlGCM_elevation = control_elev.orog[::downsample_interval, ::downsample_interval]
        controlGCM_elevation = np.nan_to_num(controlGCM_elevation)
